...
from reii.config import (
    ...
    SUBTLEX_PATH,
    USE_COREF,
    USE_GENSIM_EMBEDDINGS,
    USE_SUBTLEX,
    USE_WORDNET_QUANTIFIERS,
    WORD_EMBEDDINGS_PATH,
)
from reii.lang.es import (
    ADVERB_BLOCKED,
    ADVERB_CATEGORIES,
    ALL_KNOWN_ADVERBS,
...
class Config:
    ...
    word_embeddings_path: Optional[str] = WORD_EMBEDDINGS_PATH
    gliner_model: str = GLINER_MODEL
    use_gensim_embeddings: bool = USE_GENSIM_EMBEDDINGS
    gpt2_model: str = GPT2_MODEL
    sentence_embedder_model: str = SENTENCE_EMBEDDER_MODEL
    adverb_classifier_dir: str = ADVERB_CLASSIFIER_DIR
    stanza_lang: str = STANZA_LANG
    stanza_use_gpu: bool = STANZA_USE_GPU
    adverb_confidence_threshold: float = 0.5
    adverb_cache_size: int = 10_000
    subtlex_path: Optional[str] = SUBTLEX_PATH
    use_subtlex: bool = USE_SUBTLEX
    use_surprisal: bool = False
    use_coref: bool = USE_COREF
    use_wordnet_quantifiers: bool = USE_WORDNET_QUANTIFIERS
...
class NLPProvider:
    ...
    @classmethod
    def get_nlp(cls, model_name: str | None = None) -> spacy.Language:
...
def ner(
    ...
    entities = gliner_model.predict_entities(span.text, labels, threshold=threshold)
...
def train_adverb_classifier(
    output_dir: str, model_name: str | None = None, use_lexicon_hints: bool = True
):
    if model_name is None:
        model_name = SENTENCE_EMBEDDER_MODEL
...
class PipelineGramatical:
    def __init__(self, config: Config):
        ...
        self.adverb_cache = {}
        self.gliner_model = GLiNER.from_pretrained(config.gliner_model).to(
            config.device
        )
