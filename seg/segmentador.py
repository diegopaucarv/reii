...
class ProgressiveSegmenter:
    ...
    def recursive_segmentation(self, sentences, depth=0):
        ...
        if len(sentences) > 500:
            self._dprint(f"[RecSeg] {len(sentences)} sentences at depth {depth} — switching to windowed segmentation.")
    ...
    def _windowed_similarity(self, embeddings: np.ndarray, window: int = 10) -> np.ndarray:
        """
    ...
    def progressive_clustering(self, segments):
        ...
        for i in range(1, len(segments)):
            ...
            MAX_HISTORY = 200
            for attr in ("similarity_history", "syntactic_diff_history", "lex_sim_history"):
    ...
    def _extract_global_chains(self, segments_info, full_doc_text, context_units=2):
        ...
        for uce_idx, uce in enumerate(segments_info):
            ...
            if window_char_len > MAX_WINDOW_CHARS and len(buffer) > 1:
                ...
                while shrunk_buffer and len(full_doc_text[shrunk_buffer[0][0]:shrunk_buffer[-1][1]]) > MAX_WINDOW_CHARS:
                ...
                if shrunk_buffer:
                    ...
                    self._dprint(f"    Ventana truncada a {MAX_WINDOW_CHARS} chars (fallback).")
            ...
            try:
                ...
                self._dprint(f"    MemoryError en ventana UCE {uce_idx + 1} — saltando.")
                gc.collect()
                continue
            except Exception as e:
                logging.error(
